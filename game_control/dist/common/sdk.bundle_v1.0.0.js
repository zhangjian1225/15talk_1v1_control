    ...
    */
    ...
    setGlobalData: function (json) {
        ...
        if (json.data.length > 0) {
            var userFun = 'this.adaptData';
            if (json.operate == '5') {
                userFun = 'this.setData';
            } else {
                userFun = 'this.adaptData'
            } 
            ...
            case '5':
                if (json.data) {
                    json.data.forEach(function (item) {
                        ...
                        // item.value.starSend = '1';
                        fixJson.data = [];
                        fixJson.data.push(item);
                        if (item.value.type == 'sync') {
                            router.setData(fixJson, {
                                'type': 'sync'
                            });
                        } else if (item.value.type == 'resultSyncStu') {
                            router.setData(fixJson, {
                   ...
                   // 筛选有用的包
                   router.setFilterArr(router.allDAtaArr)
                };
                break;
        }
    },
    ...
    setFilterArr: function (arr) {
        ...
        for(var i= 0; i<arr.length; i++){
            var arrItem = JSON.parse(arr[i]);
            if(window.sendOrReceive.localIdNum == window.gTypes.length){
                window.sendOrReceive.localIdNum = arrItem.data[0].value.localIDNum;
            } else if(arrItem.data[0].value.localIDNum > window.sendOrReceive.localIdNum){
                window.sendOrReceive.localIdNum = arrItem.data[0].value.localIDNum;
            }
            if(arrItem.type== "clickCard") {
                currentCardId = arrItem.data[0].value.cardData.id;
            }
            if(arrItem.type== "coursePaging") {
                currentCardPageNum = arrItem.data[0].value.currentPage;
            }
            if (arrItem.type== "sync") {
                arrItem.data[0].value.receiveUser = SDK.classConf.user.id;
                syncArr.push(arrItem);
            } else {
                runArr.push(arrItem);
            }
        }
        ...
        console.log('-------------------courseQueueArr------------------',courseQueueArr);
        //执行筛选之后的数据
    ...
    setData: function (json, obj) {
        var data = $.extend(json, obj);
        router.allDAtaArr.push(JSON.stringify(data));
    },
...
var logicUnpack = {
    ...
    gSetData: function (obj) {
        if (obj.data && obj.data.length) {
            globalCommon.logOnlinePrint('下发全局设置协议', obj);
    ...
    memberChange: function (obj) {
        globalCommon.logOnlinePrint('下发成员变更协议', obj);
    ...
    //星星通知协议
    starData: function (obj) {
        globalCommon.logOnlinePrint('下发星星通知协议', obj);
    ...
    pageData: function (obj) {
        if (obj.curPage) {
            globalCommon.logOnlinePrint('下发翻页协议', obj);
    ...
    //通用通知协议
    notifyData: function (obj) {
        globalCommon.logOnlinePrint('下发通用通知协议', obj);
        SDK.router.setGeneralNoticeData(obj);
    },

    //画笔颜色初始化
    toolsColorSet: function (obj) {
        globalCommon.logOnlinePrint('下发画笔颜色协议', obj);
