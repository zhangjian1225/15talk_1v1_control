    ...
    for (i = pi; i < runLen; i++) {
      ...
      for (j = pj; j < childLen; j++) {
        ...
        if (type != undefined && value != undefined) {
          if (["wbAddData","wbDelData","wbEditData","wbClearData"].indexOf(type)>-1) {
            type = "wbData";
          }else if(["clAddData","clDelData","clEditData","clClearData"].indexOf(type)>-1){
            type = "clData"
          }
    ...
    try {
      ...
      if (h5JSConf.jsToCTimer == 0) {
        ...
        h5JSConf.jsToCTimer = window.setTimeout(function () {
          try {
            var targetData = h5JSConf.jsToCArr.splice(0);
            if (targetData) {
              let tar1 = targetData.map(function (item) {
                return {
                  "type": item.type,
                  "value": JSON.stringify(item.value)
                }
              })
              localStorage.setItem('mysync', MyBase64.encode(JSON.stringify(tar1)));
            }
