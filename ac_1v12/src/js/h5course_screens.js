...
var screenAc = {
  ...
  addStuScreen: function (param) {
    ...
    z += '<iframe class="frame_control frame" frame_load_status="0" user_type="' + param.role + '" user_id="' + param.uid + '" role="' + param.uid + '" src="" scrolling="no"></iframe>';
  ...
  initScreensByUser: function (userType) {
    if (userType == 'stu') {
      ...
      let screenPages = $('#h5_course_screen_pages');
