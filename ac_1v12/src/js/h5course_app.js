...
let App = {
  ...
  bindEvents: function () {
    ...
    let screenFrames = $('#h5_course_stu_screens .frame');
    let showStuButton = $('#h5_btn_showStu');

    selfFrame.off('load').on('load', function () {
      ...
      if ((classConf.h5Course.pageLoadEndTime - classConf.h5Course.pageLoadStartTime >= 20000) || $(this)[0].contentWindow.SDK == undefined) {
        ...
        if (hasFourWindow == '1') {
          ...
          if (screenFrames.length > 0) {
            screenFrames.attr('src', pageUrl);
            //监听screen frame 加载情况
            screenFrames.each(function () {
          ...
          screenFrames.attr('src', '');
          screenFrames.attr('frame_load_status', '1');
        }
